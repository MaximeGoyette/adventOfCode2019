...
def print_grid():
    global grid, position, direction
    w, h = 50, 7
    ox, oy = 0, 0
    robot_directions = ['^', '<', 'v', '>']
    grid_to_print = [['.' for _ in range(w)] for _ in range(h)]
    for pos, color in grid.items():
        grid_to_print[pos[1] + oy][pos[0] + ox] = '.' if color == 0 else '#'
    grid_to_print[position[1] + oy][position[0] + ox] = robot_directions[direction]
    print('\033[2J')
